/**
 * INDOBID — TOP ENGAGEMENT RANKER
 * Ranks active debates by genuine user engagement.
 *
 * Core Invariants:
 * 1. Engagement combines verified likes, comments/contributions, and reach.
 * 2. Ranked by likeCount DESC, contributionCount DESC, impressionCount DESC.
 * 3. Ties broken by totalVerifiedContribution DESC, then createdAt DESC.
...
export interface TopEngagementQueryOptions {
  category?: string;
  page?: number;
  limit?: number;
...
export interface TopEngagementResult {
...
export class TopEngagementRanker {
  async getTopEngagementDebates(options: TopEngagementQueryOptions = {}): Promise<TopEngagementResult> {
    const {
      category = 'all',
      page = 1,
      limit = 20,
...
export const topEngagementRanker = new TopEngagementRanker();
