/**
 * INDOBID — CREATOR EARNINGS SERVICE
...
export class EarningsService {
  ...
  async calculateDebateReward(
    ...
    const debate = await debateRepository.findByIdWithDetails(debateId);
  ...
  async calculateCreatorEconomicsBreakdown(
    ...
    const userDebates = await debateRepository.findMany({
      where: {
    ...
    for (const debate of userDebates) {
      for (const c of (debate as any).contributions || []) {
    ...
    const ledgerEntries = await ledgerRepository.findMany({
  ...
  async reverseCreatorReward(
    ...
    try {
      const ledgerEntry = await ledgerRepository.findByContributionId(contributionId);

      if (!ledgerEntry || ledgerEntry.status === 'reversed') {
        return { success: false, reversedAmountPaise: 0 };
      }

      await ledgerRepository.updateByContributionId(contributionId, {
        status: 'reversed',
        reversedAt: new Date(),
